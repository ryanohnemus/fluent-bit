                          ...
                          uint64_t *max_resource_version, flb_sds_t *continue_token)
{
    ...
    const flb_sds_t action = "ADDED"; /* All items from a list we consider an 'ADDED' type */
...
static struct flb_http_client *make_event_watch_api_request(struct k8s_events *ctx,
    ...
    //flb_sds_printf(&url, "?watch=1&resourceVersion=%d", max_resource_version);
    flb_sds_printf(&url, "?watch=1");
...
static int initialize_http_client(struct flb_http_client* c, struct k8s_events* ctx)
{
                              ...
                              struct flb_config *config, void *in_context)
{
    ...
    while( ret == FLB_HTTP_MORE || ret == FLB_HTTP_CHUNK_AVAILABLE ) {        
        ...
        if( ret == FLB_HTTP_CHUNK_AVAILABLE ) {
            //todo: process result
            ret2 = process_http_chunk(ctx, c, &bytes_consumed);
        }
    }
    // TODO: process any remaining data on socket close?


    //Chunks will go to the callback as they complete
    if (c->resp.status != 200) {
        flb_plg_warn(ins, "http watch failure, http_status=%d payload=%s", c->resp.status, c->resp.payload);
    }
